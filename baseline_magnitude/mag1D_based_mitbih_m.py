...
x = Conv1D(32,10, activation=None , padding='same', kernel_regularizer=regularizers.l2(0.0005))(x_)
x = BatchNormalization()(x)
x = relu(x)

x = Conv1D(32,10, activation=None , padding='same', kernel_regularizer=regularizers.l2(0.0005))(x)
x = BatchNormalization()(x)
x = relu(x)
x = MaxPooling1D(2,strides=2)(x)

x = Conv1D(64,10, activation=None , padding='same', kernel_regularizer=regularizers.l2(0.0005))(x)
x = BatchNormalization()(x)
x = relu(x)
...
#x = Conv1D(64,4, activation=None , padding='same')(x)
#x = BatchNormalization()(x)
#x = relu(x)
#x = MaxPooling1D(4,strides=4)(x)
...
x = Dense(80, activation=None, kernel_regularizer=regularizers.l2(0.0005))(x)
x = BatchNormalization()(x)
x = relu(x)
#x = Dropout(0.6)(x)

y = Dense(2, activation='softmax', kernel_regularizer=regularizers.l2(0.0005))(x)
...
pruning_params = {
    'pruning_schedule': pruning_schedule.ConstantSparsity(0.75, 0),
    'block_size': (1, 1),
...
pruned_model.compile(optimizer=optimizer, loss='categorical_crossentropy', metrics=['categorical_accuracy'])
...
pruned_model.fit(x=x_train_, y=y_train_, batch_size = 32, callbacks=callbacks,epochs=15, verbose=1)

pruned_model.evaluate(x=x_test_, y=y_test_, batch_size = 32)

y_pred = pruned_model.predict(x_test_)
