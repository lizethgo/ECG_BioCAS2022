...
def preprocessing(selector):
    ...
    *******************************************************************************
    ...
    synthetic data
    """
    rep = 5 ## number of times data is generated for each label = 1
...
pruning_params = {
    'pruning_schedule': pruning_schedule.ConstantSparsity(0.91, 0),
    'block_size': (1, 1),
...
pruned_model.fit(x=x_train_images, y=y_train_, batch_size = 32, callbacks=callbacks,epochs=35, verbose=1)

pruned_model.evaluate(x=x_test_images, y=y_test_, batch_size = 32)

y_pred = pruned_model.predict(x_test_images)
