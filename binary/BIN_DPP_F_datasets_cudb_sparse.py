...
def preprocessing(selector):
    ...
    *******************************************************************************
    ...
    synthetic data
    """
    rep = 6 ## number of times data is generated for each label = 1
...
x_train_, x_test_, y_train_, y_test_ = preprocessing(selector=2)
...
from sparseconnect_filters_q import sparseconnect_layer, sparseconnect_CNN_layer
from sparseconnect_bin import bin_sparseconnect_layer, bin_sparseconnect_CNN_layer, BinaryLayer
...
#x = BatchNormalization()(x)
#x = Activation('relu')(x)
x = bin_sparseconnect_layer(100, 180 ,activation='softmax',n_epochs=33, tempIncr=5)(x)
#x = BatchNormalization()(x)
#x = relu(x)
#x = Dropout(0.4)(x)

y = BinaryLayer(2, activation='softmax')(x)
...
#
model.fit(x=x_train_images__, y=y_train__, batch_size = 64, epochs=50, verbose=1, validation_data=(x_val,y_val))
