...
def preprocessing(selector):
    ...
    *******************************************************************************
    ...
    synthetic data
    """
    rep = 9 ## number of times data is generated for each label = 1
...
x_train_, x_test_, y_train_, y_test_ = preprocessing(selector=3)

#x_train_ = (x_train_-np.mean(x_train_))/(np.std(x_train_)+1e-7) 
#x_test_ = (x_test_-np.mean(x_test_))/(np.std(x_test_)+1e-7)
...
print('MAX', np.max(np.abs(x_train_images)))    
...
#### validation 
val_ = 0.1
print('SIZE   -- ', np.shape(x_train_images)[0])
print('SIZE   -- ', np.shape(y_train_)[0])
x_val = x_train_images[0:int(val_*np.shape(x_train_images)[0]),:,:,:]
y_val = y_train_[0:int(val_*np.shape(y_train_)[0]),:]

x_train_images__ = x_train_images[int(val_*np.shape(x_train_images)[0]):,:,:,:]
y_train__ = y_train_[int(val_*np.shape(y_train_)[0]):,:]
...
from sparseconnect_filters_u import sparseconnect_layer, sparseconnect_CNN_layer_filter, sparseconnect_CNN_layer
...
# FINAL MODEL (REPORTES)
# x_ = Input(shape=(N_in_height, N_in_length, np.shape(x_train_images)[-1]))
# x = Conv2D(8,(3,3), activation=None , padding='same')(x_)
...
# # x = sparseconnect_CNN_layer_filter(n_connect_ch =61, 
                              # # filters = 512, 
...
# #x = sparseconnect_layer(512, 200 ,activation='relu',n_epochs=10, tempIncr=5)(x)
# #x = BatchNormalization()(x)
# #x = Activation('relu')(x)
# x = sparseconnect_layer(30, 60 ,activation='softmax',n_epochs=10, tempIncr=5)(x)
# x = BatchNormalization()(x)
# x = relu(x)
# #x = Dropout(0.4)(x)

# x = sparseconnect_layer(2, 10 ,activation='softmax',n_epochs=10, tempIncr=5)(x)
# x = BatchNormalization()(x)
...
#x = BatchNormalization()(x)
#x = Activation('relu')(x)
x = sparseconnect_layer(100, 60 ,activation='softmax',n_epochs=10, tempIncr=5)(x)
x = BatchNormalization()(x)
x = relu(x)
#x = Dropout(0.4)(x)
...
#
model.fit(x=x_train_images__, y=y_train__, batch_size = 64, epochs=120, verbose=1, validation_data=(x_val,y_val))  ##45 for final model 

model.evaluate(x=x_test_images, y=y_test_, batch_size = 64)
