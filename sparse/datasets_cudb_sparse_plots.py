...
x = sparseconnect_CNN_layer_filter(n_connect_ch =60, 
                              filters = 256, 
                              kernel_size = kernel_size,
                              channel_size = 128,
                              activation='relu',
...
#x = BatchNormalization()(x)
#x = Activation('relu')(x)
x = sparseconnect_layer(512, 70 ,activation='softmax',n_epochs=10, tempIncr=5)(x)
x = BatchNormalization()(x)
x = relu(x)
x = Dropout(0.4)(x)
...
#
model.fit(x=x_train_images, y=y_train_, batch_size = 32, epochs=25, verbose=1)
