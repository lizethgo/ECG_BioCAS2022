...
class DPS_topk(Layer):
    ...
    def call(self, inp):
        ...
        if self.is_CNN == True:
            ...
            inp = tf.reshape(inp, [inp.shape[0], (inp.shape[-2])*(inp.shape[-2]), inp.shape[-1]])
...
class sparseconnect_layer(Layer):   
    ...
    def call(self, x):
        ...
        if self.one_per_batch:
            ...
            A = DPS_topk(is_CNN = False, is_CNN_1D = False, BS=1, k = self.n_connect, n_epochs=self.n_epochs,  tempIncr=self.tempIncr)(self.D)
...
class sparseconnect_CNN_layer_1D(Layer): 
    ...
    def call(self, x):
        ...
        print('x shape', np.shape(x))
        print('AW shape', np.shape(AW))
        y = tf.nn.conv1d(x, AW, self.strides, padding = 'SAME')
        y = tf.nn.bias_add(y,self.b)      
...
class sparseconnect_CNN_layer_filter_1D(Layer): 
    """
    Sparse convolutional layer (filter level)
    - Generates trainable logits (D_ch)
    - Call DPS_topK to perform optimization
    ...
    def __init__(self,
        ...
        super(sparseconnect_CNN_layer_filter_1D, self).__init__(name=name, **kwargs) 
    ...
    def build(self, input_shape): 
        ...
        super(sparseconnect_CNN_layer_filter_1D, self).build(input_shape) 
    ...
    def call(self, x):
        ...
        if self.one_per_batch:
            ...
            print('A CH SHAPE', A_ch.shape)
            A_ch = tf.reduce_sum(A_ch,axis=-2)
            A_ch = tf.reshape(A_ch,[1,A_ch.shape[-1], A_ch.shape[-2]])
            #aux = tf.ones(shape=(y.shape[1], y.shape[2], y.shape[3]))
            ########
            ...
            #########
            y = tf.nn.conv1d(x, A_ch, self.strides, padding = 'SAME')
            y = tf.nn.bias_add(y,self.b)  
