...
class DPS_topk(Layer):
    ...
    def call(self, inp):
        ...
        if self.is_CNN == True:
            ...
            inp = tf.reshape(inp, [inp.shape[-1], inp.shape[0],(inp.shape[-2])*(inp.shape[-2])])
...
class sparseconnect_CNN_layer(Layer): 
    ...
    def call(self, x):
        ...
        scale = ((2.**3-1)/2)/tf.math.reduce_max(tf.abs(AW))
        #tensor_1 = tf.clip_by_value(tf.round(scale), -127, 127)
        tensor_1 = tf.round(scale*AW)
...
class sparseconnect_CNN_layer_filter(Layer): 
    ...
    def call(self, x):
        ...
        if self.one_per_batch:
            ...
            #########
            scale = ((2.**3-1)/2)/tf.math.reduce_max(tf.abs(A_ch))
