...
from sparseconnect_filters import sparseconnect_layer, sparseconnect_CNN_layer_filter, sparseconnect_CNN_layer
...
x = sparseconnect_CNN_layer_filter(n_connect_ch = 4, 
                              filters = 20, 
...
x = sparseconnect_CNN_layer_filter(n_connect_ch =8, 
                              filters = 20, 
                              kernel_size = kernel_size,
                              channel_size = 20,
                              activation='relu',
...
#x = BatchNormalization()(x)
#x = Activation('relu')(x)
x = sparseconnect_layer(30, 30 ,activation='softmax',n_epochs=10, tempIncr=5)(x)
x = BatchNormalization()(x)
x = relu(x)
#x = Dropout(0.4)(x)

x = sparseconnect_layer(2, 15 ,activation='softmax',n_epochs=10, tempIncr=5)(x)
x = BatchNormalization()(x)
...
#
model.fit(x=x_train_images, y=y_train_, batch_size = 64, epochs=30, verbose=1)
