...
def preprocessing(selector=1):
    ...
    *******************************************************************************
    ...
    synthetic data
    """
    rep = 75 ## number of times data is generated for each label = 1
...
#x = BatchNormalization()(x)
#x = Activation('relu')(x)
x = sparseconnect_layer(100, 80 ,activation='softmax',n_epochs=33, tempIncr=5)(x)
x = BatchNormalization()(x)
x = relu(x)
#x = Dropout(0.4)(x)
...
#
model.fit(x=x_train_images__, y=y_train__, batch_size = 64, epochs=45, verbose=1, validation_data=(x_val,y_val))
