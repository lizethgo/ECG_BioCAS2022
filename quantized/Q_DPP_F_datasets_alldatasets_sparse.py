...
#x_train_, x_test_, y_train_, y_test_ = preprocessing(selector=4)
...
from sparseconnect_filters_q import sparseconnect_layer,  sparseconnect_CNN_layer, sparseconnect_CNN_layer_1D
...
x_ = Input(shape=(N_in_height, N_in_length))
#
x = Conv1D(32,10, activation=None , padding='same')(x_)
x = BatchNormalization()(x)
x = relu(x)
x = MaxPooling1D()(x)
...
x = sparseconnect_layer(100, 50 ,activation='relu',n_epochs=100, tempIncr=5)(x)
x = BatchNormalization()(x)
x = relu(x)
#x = Dropout(0.4)(x)

x = sparseconnect_layer(2, 20 ,activation='softmax',n_epochs=100, tempIncr=5)(x)
x = BatchNormalization()(x)
...
model.fit(x=x_train_, y=y_train_, batch_size = 64, epochs =25, verbose=1)
